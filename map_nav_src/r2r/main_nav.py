...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            IL_loss = sum(listner.logs['IL_loss']) / max(len(listner.logs['IL_loss']), 1) # avg IL_loss (SAP+PID) in interval
            ...
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, low_RL_loss %.4f, high_RL_loss %.4f, policy_loss %.4f, low_critic_loss %.4f, high_critic_loss %.4f" % (
                    total, length, entropy, IL_loss, low_RL_loss, high_RL_loss, policy_loss, low_critic_loss, high_critic_loss),
                record_file
            )
        ...
        if default_gpu:
            ...
            if args.submit:
                ...
                if args.vis:
                    json.dump(
                        vis_record,
                        open(os.path.join(args.pred_dir, "%s_%s_vis.json" % (prefix, env_name)), 'w'),
                        sort_keys=True, indent=4, separators=(',', ': ')
                    )
