...
class BaseAgent(object):
    ...
    def test(self, iters=None, **kwargs):
        ...
        if iters is not None:
            ...
            for i in range(iters):
                for traj in self.rollout(is_train = False, **kwargs):
                    self.loss = 0
    ...
    def test_viz(self, iters=None, **kwargs):
        ...
        if iters is not None:
            ...
            while True:
                for traj in self.rollout_viz(is_train = False, **kwargs):
                    if traj['instr_id'] in self.results:
                        looped = True
                    else:
                        self.loss = 0
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        if self.args.world_size > 1:
            ...
            self.critic_high = DDP(self.critic_high, device_ids=[self.rank], find_unused_parameters=True)

        self.models = (self.vln_bert, self.critic, self.critic_high)
...
#        if self.args.zoner != 'null':
#            self.zoner_optimizer = optimizer(self.vln_bert.vln_bert.zoner.parameters(), lr=self.args.lr)
#            self.critic_high_optimizer = optimizer(self.critic_high.parameters(), lr=self.args.lr)
#            self.optimizers = (self.vln_bert_optimizer, self.critic_optimizer, self.zoner_optimizer, self.critic_high_optimizer)
#        else:
#            self.optimizers = (self.vln_bert_optimizer, self.critic_optimizer)
    ...
    def test(self, use_dropout=False, feedback='argmax', allow_cheat=False, iters=None, viz=False):
        ...
        if use_dropout:
            self.vln_bert.train()
            self.critic.train()
            self.critic_high.train()
        else:
            self.vln_bert.eval()
            self.critic.eval()
            self.critic_high.eval()
        if viz:
    ...
    def train(self, n_iters, feedback='teacher', **kwargs):
        ...
        for iter in range(1, n_iters + 1):
            ...
            self.critic_high_optimizer.zero_grad()
            ...
            if self.args.train_alg == 'imitation':
                ...
                self.rollout(
                    train_ml=1., train_rl=False, is_train=True, **kwargs
                )
            elif self.args.train_alg == 'dagger': 
                if self.args.ml_weight != 0:
                    self.feedback = 'teacher'
                    self.rollout(
                        train_ml=self.args.ml_weight, train_rl=False, is_train=True, **kwargs
                    )
                self.feedback = 'expl_sample' if self.args.expl_sample else 'sample'
                self.rollout(train_ml=1, train_rl=False, is_train=True, **kwargs)
            elif self.args.train_alg == 'iterative':
                if self.args.ml_weight != 0:
                    self.feedback = 'teacher'
                    self.rollout(
                        train_ml=self.args.ml_weight, train_rl=False, is_train=True, **kwargs
                ...
                self.feedback = 'expl_sample' if self.args.expl_sample else 'sample'
                if self.args.rl_weight != 0:
                    self.rollout(train_ml=1, train_rl=self.args.rl_weight, is_train=True, **kwargs)
                else:
                    self.rollout(train_ml=1, train_rl=False, is_train=True, **kwargs)
            else:
                if self.args.ml_weight != 0:
                ...
                self.rollout(train_ml=None, train_rl=True, is_train=True, **kwargs)
            ...
            # high-level task
            if self.args.zoner != "null":
                self.zoner_optimizer.step()
                self.critic_high_optimizer.step()
