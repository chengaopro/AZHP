...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if args.eval_first:
        ...
        for env_name, env in val_envs.items():
            ...
            if args.submit:
                output_file = os.path.join(args.pred_dir, "%s_%s_%s.json" % ('submit_eval_first', env_name, args.fusion))
                json.dump(
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            length = max(len(listner.logs['low_critic_loss']), 1)   # RL: total (max length) in the batch
            low_critic_loss = sum(listner.logs['low_critic_loss']) / total
            high_critic_loss = sum(listner.logs['high_critic_loss']) / total
            policy_loss = sum(listner.logs['policy_loss']) / total
            low_RL_loss = sum(listner.logs['low_RL_loss']) / max(len(listner.logs['low_RL_loss']), 1) # avg RL_loss in interval
            high_RL_loss = sum(listner.logs['high_RL_loss']) / max(len(listner.logs['high_RL_loss']), 1) # avg RL_loss in interval            
            OG_loss = sum(listner.logs['OG_loss']) / max(len(listner.logs['OG_loss']), 1)
            IL_loss = sum(listner.logs['IL_loss']) / max(len(listner.logs['IL_loss']), 1)
            Zone_selection_loss = sum(listner.logs['Zone_selection_loss']) / max(len(listner.logs['Zone_selection_loss']), 1)
            Zone_partition_loss = sum(listner.logs['Zone_partition_loss']) / max(len(listner.logs['Zone_partition_loss']), 1)
            entropy = sum(listner.logs['entropy']) / total
            rezone_ratio = sum(listner.logs['rezone_ratio']) / max(len(listner.logs['rezone_ratio']), 1)
            predict_rezone_ratio = sum(listner.logs['predict_rezone_ratio']) / max(len(listner.logs['predict_rezone_ratio']), 1)
            writer.add_scalar("loss/low_critic", low_critic_loss, idx)
            writer.add_scalar("loss/high_critic", high_critic_loss, idx)
            writer.add_scalar("policy_entropy", entropy, idx)
            writer.add_scalar("loss/low_RL_loss", low_RL_loss, idx)
            writer.add_scalar("loss/high_RL_loss", high_RL_loss, idx)
            writer.add_scalar("loss/OG_loss", OG_loss, idx)
            writer.add_scalar("loss/IL_loss", IL_loss, idx)
            writer.add_scalar("loss/Zone_selection_loss", Zone_selection_loss, idx)
            writer.add_scalar("loss/Zone_partition_loss", Zone_partition_loss, idx)
            writer.add_scalar("total_actions", total, idx)
            writer.add_scalar("zoner/rezone_ratio", rezone_ratio, idx)
            writer.add_scalar("zoner/predict_rezone_ratio", predict_rezone_ratio, idx)
            writer.add_scalar("max_length", length, idx)
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, OG_loss %.4f, policy_loss %.4f, low_RL_loss %.4f, high_RL_loss %.4f, low_critic_loss %.4f, high_critic_loss %.4f" % (
                    total, length, entropy, IL_loss, OG_loss, policy_loss, low_RL_loss, high_RL_loss, low_critic_loss, high_critic_loss),
                record_file
            )
        ...
        for env_name, env in val_envs.items():
            ...
            if default_gpu:
                ...
                if env_name == 'val_seen':
                    if score_summary['rgspl'] <= 52:
                        is_save = is_save + 1
                if env_name == 'val_unseen':
                    if score_summary['rgspl'] >= 23:
                        is_save = is_save + 1
        ...
        if is_save == 2:
            listner.save(idx, os.path.join(args.ckpt_dir, "ckpt_%s" % (str(idx))))
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    from models.vis import vis_record, vis_scans
    for env_name, env in val_envs.items():
        ...
        output_file_vis = os.path.join(args.pred_dir, "%s_%s_%s_vis.json" % (
            prefix, env_name, args.fusion))
...
#            continue
        ...
        if default_gpu:
            ...
            if args.submit:
                ...
                if args.vis:
                    json.dump(
                        vis_record, open(output_file_vis, 'w'),
                        sort_keys=True, indent=4, separators=(',', ': ')
                    )
