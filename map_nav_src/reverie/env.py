...
class ReverieObjectNavBatch(object):
    ...
    def _next_minibatch(self, batch_size=None, **kwargs):
        ...
        if self.multi_endpoints:
            for i, item in enumerate(batch):
                if 'objId' in item and item['objId'] is not None: # official split
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            ob = {
                ...
                'instr_clip_encoding': item['instr_clip_encoding'],
                'instr_clip_length': item['instr_clip_length'],
                'gt_path' : item['path'],
    ...
    def reset(self, **kwargs):
        ...
        headings = [item.get('heading', 0) for item in self.batch]
