...
def gen_seq_masks(seq_lens, max_len=None, zone_ops=False, gmap_vp_zones=None, zone_label=None):
    if max_len is None:
    ...
    if zone_ops:
        for i in range(batch_size):
            for j in range(1, len(gmap_vp_zones[i])):
                if gmap_vp_zones[i][j] != zone_label[i]:
                    masks[i][j] = False
    return masks
