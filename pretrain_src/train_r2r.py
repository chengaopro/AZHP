...
def main(opts):
    ...
    if opts.cache_dir:
        tokenizer = AutoTokenizer.from_pretrained(opts.cache_dir)
    else:
    ...
    if opts.checkpoint:
        ...
        if opts.init_pretrained == 'bert':
            ...
            tmp = torch.load(
                opts.data_root_dir + '/pretrained/LXMERT/model_LXRT.pth' if opts.data_root_dir else 'datasets/pretrained/LXMERT/model_LXRT.pth', 
            ...
            for param_name, param in tmp.items():
                ...
                if 'bert.encoder.layer' in param_name:
                    ...
                    checkpoint[param_name] = param
            #print(tmp['module.bert.embeddings.word_embeddings.weight'])
            del tmp
...
def build_args():
    ...
    if os.path.exists(opts.output_dir) and os.listdir(opts.output_dir):
        ...
        if os.path.exists(os.path.join(opts.output_dir, '.torch_distributed_sync')):
            os.remove(os.path.join(opts.output_dir, '.torch_distributed_sync'))
